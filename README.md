...
2. Демону, для большей стабильности, следует иметь собственное локальное хранилище(файл или БД). Реализованного сейчас механизма передачи Менеджеру промежуточных результатов может оказаться недостаточно. Пуллинг заданий вспомогательного цикла, т.е. заданий с "condition=2" - это также плохое решение на мой взгляд.
3. Если двигаться в сторону Целевой Архитектуры, то ясно, что взаимодействие с Демоном(ми) должно быть реализовано с применением очередей(брокеров сообщений). Кроме того, использование большего числа Демонов и Менеджеров потребует реализации соответствующей логики управления. 
4. Доработка Диспетчера, такая как использование отдельных горутин для обработки POST и GET запросов(или даже использование для каждого из этих видов запросов отдельного приложения), а также отдельных горутин для выполнения запросов к БД - хорошая идея, но не первоочередная, т.к. в текущей реализации Диспетчер и так может быть хорошо горизонтально масштабирован. В первую очередь переработки требует Менеджер. 
